var chatService = abp.services.qks.chat;
var friendshipService = abp.services.qks.friendship;

var appSession = {
    load: function (callback) {
        abp.services.qks.session.getCurrentLoginInformations({ async: false })
    ...
    getFormattedMessages: function (messages) {
        $.each(messages, function (index, message) {
            ...
            if (message.message.startsWith('[image]')) {
                ...
                var imageUrl = abp.appPath + 'Admin/Chat/GetImage?id=' + message.id + '&contentType=' + image.contentType;
    ...
    user: {
        ...
        openSearchModal: function (tenantId) {
            var lookupModal = app.modals.LookupModal.create({
                ...
                serviceMethod: abp.services.qks.commonLookup.findUsers,
